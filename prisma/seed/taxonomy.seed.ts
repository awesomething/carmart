...
export async function seedTaxonomy(prisma: PrismaClient) {
  ...
  const makePromises = Object.entries(result).map(([name]) => {
    return prisma.make.upsert({
      where: { name },
      create: { name, image: `https://vl.imgix.net/img/${name.replace(/\s*/g, '-').toLowerCase()}-logo.png?auto=format, compress` },
      update: { name, image: `https://vl.imgix.net/img/${name.replace(/\s*/g, '-').toLowerCase()}-logo.png?auto=format, compress` },
    })
  });
